...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    """
    probs.set_shape([None, config.NUM_CLASSES])
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    s = K.int_shape(x)
    if s[1]==None:
        mrcnn_bbox = KL.Reshape((-1, num_classes, 4), name="mrcnn_bbox")(x) #s[1]
    else:
        mrcnn_bbox = KL.Reshape((s[1], num_classes, 4), name="mrcnn_bbox")(x) #s[1]
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        input_image_meta = KL.Input(shape=[config.IMAGE_META_SIZE[0]],
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        #filepath = "/home/vincitech/Documents/sd-maskrcnn-master/models/sd_maskrcnn.h5"
         ...
         #   if name not in self.keras_model.metrics_names:
          #      layer = self.keras_model.get_layer(name)
           #     loss = tf.reduce_mean(layer.output, keepdims=True) * self.config.LOSS_WEIGHTS.get(name, 1.)
            #    layer.add_metric(loss, name=name)
        for name in loss_names:
            if name not in self.keras_model.metrics_names:
                layer = self.keras_model.get_layer(name)
                loss = tf.reduce_mean(layer.output, keepdims=True) * self.config.LOSS_WEIGHTS.get(name, 1.)
                metric = tf.keras.metrics.Mean(name=name)
                layer.add_metric(metric(loss), aggregation='mean')
