...
class MyLayer(tf.keras.layers.Layer):
    def call(self, x):
        return tf.reduce_mean(x, keepdims=True)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    shared = KL.Lambda(lambda x: K.squeeze(K.squeeze(x, 3), 2),
                   name="pool_squeeze",
                   output_shape=lambda s: (s[0], s[1], s[4]))(x)
    ...
    while True:
        try:
            ...
            if b >= batch_size:
                ...
                # Always return outputs as a tuple, even if empty
                yield inputs, tuple(outputs)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        input_image = KL.Input(
            shape=[512, 512, 3], name="input_image")
        ...
        if mode == "training":
            ...
            image_shape = K.int_shape(input_image)[1:3]
            print(config.IMAGE_META_SIZE)
            gt_boxes = KL.Lambda(lambda x: norm_boxes_graph(x, image_shape),
                     output_shape=lambda s: (s[0], None, 4))(input_gt_boxes)
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.checkpoint_path = os.path.join(self.log_dir, "mask_rcnn_{}_*epoch*.weights.h5".format(
...
def norm_boxes_graph(boxes, shape):
    ...
    """
    print(shape)
    h, w = tf.split(tf.cast(tf.convert_to_tensor(shape), tf.float32), 2)
