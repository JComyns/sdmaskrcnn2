...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        input_image = KL.Input(
            shape=[None, None, config.IMAGE_SHAPE[1]], name="input_image")
...
conf_parser.add_argument(
    "--config",
    action="store",
    default="c:/Users/comyn/OneDrive/Documents/GitHub/sd-maskrcnn/cfg/train.yaml",
    dest="conf_file",
    type=str,
...
if not os.path.exists(config["path"]):
    if mode == "inference":
        raise ValueError(
            "No model located at {}".format(config["path"])
        )
 #   else:
#        utils.mkdir_if_missing(config["path"])

image_shape = config["settings"]["image_shape"]
config["settings"]["image_min_dim"] = min(image_shape)
config["settings"]["image_max_dim"] = max(image_shape)

if mode == "inference":
    config["settings"]["gpu_count"] = 1
    config["settings"]["images_per_gpu"] = 1
mask_config = MaskConfig(config["settings"])
...
model_dir = config["path"]
#set_log_dir()
keras_model = build(mode=mode, config=config)